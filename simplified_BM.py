...
def main():
	table = PrettyTable()
	table.field_names = ["Rank","Simplified BM15","File Name","File Number"]
	...
	for k in range(len(lines)):
		...
		for i in range(len(vocab)):
			...
			for j in range(len(text)):
				if(text[j]==q):
					BM15[k] += Fij[k][i] * math.log2((N - ni[i] + 0.5)/(ni[i] + 0.5))
					break
	# print("BM15: ",BM15)
	...
	for k in range(len(lines)):
		...
		for i in range(len(vocab)):
			...
			for j in range(len(text)):
				if(text[j]==q):
					BM11[k] += Fij_dash[k][i] * math.log2((N - ni[i] + 0.5)/(ni[i] + 0.5))
					break
	# print("BM11: ",BM11)
